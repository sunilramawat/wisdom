...
@endphp
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-10">
          <h1 class="m-0 text-dark">Knowledge Hub </h1>
        </div><!-- /.col -->
        <div class="col-sm-2">
    ...
    <div class="container-fluid">
      <div class="box-main">
         <div class="box-main-top">
          <div class="box-main-title">Knowledge Hub List</div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered admin-table dataTable"  id="example2" >
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{{$no+1}}</td>
                    <td><img src="{{$chip->photo}}" width="20px;"> </td>
                    <td>{{$chip->title}}</td>
                    <td> {{$chip->c_name}} </td>
                    <td>
                      @if($chip->status == 0)
                    ...
                    <td><!-- <a href=""><i class="fa fa-eye" aria-hidden="true"></i></a> -->
                      <!-- <a href="{{URL('admin/partner/detail')}}/{{$chip->id}}">
                       ...
                       <a href="{{URL('admin/partner/edit')}}/{{$chip->id}}"><i class="fas fa-edit"></i></a>
                              &nbsp; -->
                      <a href="{{URL('admin/collection/edit')}}/{{$chip->id}}"><i class="fas fa-edit"></i></a>
                              &nbsp;        
...
{ 
  $("#LoadingProgress").fadeIn('fast');
    if (confirm('Are you sure you want to delete this collection?')) {
      $.ajax({
        url: "{{ URL('admin/collection/delete') }}/"+Id,
        type: "GET",
        contentType: false,
        cache: false,
        processData:false,
      success: function( data, textStatus, jqXHR ) {
        if(data == 0){
          alert('Sorry, this collection cannot be deleted.');
        }else{
          window.location.reload();
         $("#LoadingProgress").fadeOut('fast');
        }
